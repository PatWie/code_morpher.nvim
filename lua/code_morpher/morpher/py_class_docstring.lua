...
return {
  get_context_node = function(_, ts_start)
    local node = utils.findup(ts_start, "class_definition")
    return {
      action_name = "Enhance class docstring",
      available = node ~= nil
    }
  end,
  generator = function(_, text)
    ...
    local llm_output = vim.fn.system(
      "python3 " .. vim.fn.expand('$HOME/') .. script_path .. " enhance_class_docstring",
      text)
  ...
  run = function(self, ts_start, ctx)
    local func_node = utils.findup(ts_start, "class_definition")
